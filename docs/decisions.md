...
The npm package names are unscoped: `zomboid-models` (the renderer), `zomboid-models-pipeline` (the converter), `zomboid-models-element` (a Web Component), `zomboid-models-react` (a thin React wrapper), and `zomboid-models-render` (pictures from Node.js). The repository is an npm workspaces monorepo, and the published packages share one version number through changesets.
...
TypeScript in strict mode, ESM only, Node 22 or newer. three.js is a peer dependency of the renderer and is bundled into the Web Component build and into the render package's page script. Packages are built with tsup, the playground with Vite, the API reference with TypeDoc into the playground's site. Tests run with vitest on synthetic fixtures, with a coverage report for information and no threshold; tests that need a real install run only when a `PZ_DIR` environment variable is set, and the render package's tests only when its Chromium is installed. ESLint and Prettier enforce style. GitHub Actions runs the checks on Node 22 and 24 and publishes releases through changesets and npm's trusted publishing. Commits follow Conventional Commits. Everything is written in English. Documentation lives in `docs/` as Markdown.
...
- 0.7: scenes of several subjects on one ground plane, characters seated in vehicles, and the `zomboid-view` element and `ZomboidView` component names. Done. Subjects keep their own documents inside the scene; `yaw: 0` faces the camera for every kind so that a scene author need not know where a vehicle's front lies in its file; characters, animals, and items are drawn 1.5 times their file units, the factor the game applies in the world, so that the group is in proportion; the seat is the script's `inside` position with the vehicle's model offset added and X mirrored, as `BaseVehicle.getPassengerLocalPos` and the seated render path do, and everyone in a vehicle plays the one seated idle the game has.

- 0.8: the game's blob shadows under characters, animals, and vehicles; blood on held weapons through the game's overlay masks; the three light bar patterns; doors, hoods, and trunks that open and close; and the light of a time of day. Done. The shadow quads follow `calculateShadowParams` and the vehicle scripts' shadow extents. The hinged parts keep their bones and the `<part>_closing` clips in the converted model, with the closed pose as the rest pose, and the viewer plays the script's `Open` and `Close` anims at their rates when a description changes a part's state. The lighting is the game's own arithmetic (`ClimateManager`, `ErosionSeason`, `RenderSettings`) reduced to a per-channel factor on the viewer's daylight, anchored so that a clear summer afternoon is the viewer's usual look: the game's absolute values include its ambient floor and its shader's multipliers, which would make every default view dim, while the curve and the colours are what a reader recognises.
...
- 1.0: the API frozen. Done. What the version promises is written in the README: the document formats, the viewer, the element, the React component, the render package, the pipeline's configuration and commands, and the exporter's index. The catalog files stay a contract between the pipeline and the viewer of one major version, stamped with the pipeline's version so that the viewer can warn, because freezing them would freeze the renderer's internals. The game's rules moved to `zomboid-models/rules`, a subpath that follows Build 42 in minor releases; the package root keeps the viewer, the formats, the names, and the asset cache. The names from the first releases went (`character`, `setCharacter`, `<zomboid-character>`, `ZomboidCharacter`, `ViewerDocument`, the `Manifest` alias), since 0.1.0 had no users to keep them for. Node 22 is the minimum, Node 20 having reached the end of its support. The wrappers gained the options they lacked and tests; the viewer retries asset requests, reports a lost WebGL context, and keeps its frame callback to itself. The repository gained the usual files of an open source project and an API reference.

A cinematic runtime for scripted scenes was proposed and declined: the cost is high, the project is a set of building blocks, and a site that needs animated scenes can build them on the viewer's methods or in a fork.

Planned next: pictures of characters, vehicles, and animals for the README, rendered from a game install with transparent backgrounds. Pictures rendered from the game's assets are the game's assets too, so where they live (in the repository with the required wording next to them, or outside it) is still to be decided. An editor package only if a site needs one. Hosting stays a recipe in the docs.

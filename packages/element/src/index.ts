...
export type { CameraOptions, CharacterDescription, RigWarning, Viewer, ViewerMode };
...
/**
 * `<zomboid-character>` shows one character. Attributes:
 *
...
export class ZomboidCharacterElement extends HTMLElement {
  ...
  private characterValue: CharacterDescription | undefined;
  ...
  private async loadFromSrc(): Promise<void> {
    ...
    try {
      ...
      const result = validateCharacterDescription(await response.json());
